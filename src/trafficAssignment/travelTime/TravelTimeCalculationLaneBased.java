...
public class TravelTimeCalculationLaneBased 
{
	//~Variables
	//--------------------------------------------------------------------------
	...
	//~Methods
	//--------------------------------------------------------------------------
	public TravelTimeCalculationLaneBased(Network network,IloCplex cplex)
	{
...
//		System.out.println("junctions.size(): "+junctions.size());
	...
	public void byLBSTFunctionPreparationOneJunctionLaneBased(int[] linkFlowAllOD, int junctionIdex)
	{	
...
//			System.out.println(modFoesMatrix);
		...
		////////////////////////////////////////////////////////			
			...
			signalTiming.solveMe(armNumber,fixedDemandForSignal, dirOutIndex,modFoesMatrix,armDir,
	                   armLanesNum,armDirNum,
					  outLanesNum,extLinks);			
			cycleLength = signalTiming.getCycleLengthEpslon();
		}	

	}
		
	public void byLBSTFunctionOneJunction(int junctionIdex,int cycleLength,double[] oldLinkFlowAllOD)
	{
		// i denotes the junction index 
...
//		List<Link> extLinks    = junc.getExternalLinks(); //外部道路		
		...
		//////////////////////////////////////////////////////////
			...
			{
				//signalTiming.assignedFlowCalculation(itemp,demand);
				delayPerMovementCalculation(armNumber,armDir,armLanesNum,armDirNum,arms,junc,cycleLength,demand);
				////////////////////////////////////////
				//计算link travel time
				////////////////////////////////////////
...
//						System.out.println("oppTHLink.getLaneCount(): "+oppTHLink.getLaneCount());
	...
	public void delayPerMovementCalculation(int armNumber,List<ArrayList<Link>> armDir,
            List<Integer> armLanesNum,List<Integer> armDirNum, List<Vertex> arms,Junction junc, int cycleLength, double[][] demand)
	{
			...
			for (int j = 0; j < armDirNum.get(i); j++)
			{
				...
				{ 
					...
					{				
						...
						double delayPerLane = delayPerLane(laneFlow,laneSaturation,cycleLength,signalTiming.getDurationLane()[i][k],observationTime);
						...
						double degreeOfS = degreeOfSaturationCalculation(laneFlow,laneSaturation,cycleLength,signalTiming.getDurationLane()[i][k]);
